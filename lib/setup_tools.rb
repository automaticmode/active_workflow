...
module SetupTools
  ...
  def set_defaults!
    unless $config['SECRET_KEY_BASE']
      puts 'Setting up SECRET_KEY_BASE...'
      set_value 'SECRET_KEY_BASE', SecureRandom.hex(64)
    end
    set_value 'RAILS_ENV', 'production'
  end

  def set_buildpacks!
    buildpacks_path = File.expand_path(
      File.join(File.dirname(__FILE__), '..', '.buildpacks'))
    buildpacks = File.open(buildpacks_path).readlines().map(&:strip)
    puts capture('heroku buildpacks:clear')
    buildpacks.each do |buildpack|
      puts capture("heroku buildpacks:add #{buildpack}")
    end
  end
