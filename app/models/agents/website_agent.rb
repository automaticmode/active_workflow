...
module Agents
  class WebsiteAgent < Agent
    ...
    description <<-MD
      ...
      The WebsiteAgent can also scrape based on incoming messages.
      ...
      * If you specify `merge` for the `mode` option, ActiveWorkflow will retain the old payload and update it with new values.
      ...
      In this example the `*` wildcard character makes the parser to iterate through all items of the `data` array. Three messages will be created as a result.
      ...
      To extract the whole content as one message:
      ...
      Set `force_encoding` to an encoding name (such as `UTF-8` and `ISO-8859-1`) if the website is known to respond with a missing, invalid, or wrong charset in the Content-Type header.  Below are the steps used by ActiveWorkflow to detect the encoding of fetched content:
      ...
      3. When `type` is `html` or `xml`, ActiveWorkflow checks for the presence of a BOM, XML declaration with attribute "encoding", or an HTML meta tag with charset information, and uses that if found.
      4. ActiveWorkflow falls back to UTF-8 (not ISO-8859-1).
      ...
      In the `on_change` mode, change is detected based on the resulted message payload after applying this option.  If you want to add some keys to each message but ignore any change in them, set `mode` to `all` and put a DeDuplicationAgent downstream.
      ...
      * `_url_`: The URL specified to fetch the content from.  When parsing `data_from_message`, this is not set.
    ...
    message_description do
      if keys = message_keys
        "Messages will have the following fields:\n\n    %s" % [
          Utils.pretty_print(Hash[message_keys.map { |key|
                                    [key, '...']
                                  }])
        ]
      else
        'Messages will be the raw JSON returned by the URL.'
      end
    end
    ...
    def validate_options
      ...
      errors.add(:base, 'either url, url_from_message, or data_from_message are required') unless options['url'].present? || options['url_from_message'].present? || options['data_from_message'].present?
    ...
    def handle_data(body, url, existing_payload)
      ...
      if extract_full_json?
        if store_payload!(previous_payloads(1), doc)
          ...
          create_message payload: existing_payload.merge(doc)
        end
        return
      end
      ...
      old_messages = previous_payloads num_tuples
      ...
      output.each do |extracted|
        ...
        if store_payload!(old_messages, result)
          log "Storing new parsed result for '#{name}': #{result.inspect}"
          create_message payload: existing_payload.merge(result)
        end
      end
    end
    ...
    def handle_message_data(data, message, existing_payload)
      interpolation_context.stack {
        interpolation_context['_response_'] = ResponseFromMessageDrop.new(message)
        handle_data(data, message.payload['url'].presence, existing_payload)
      }
    rescue => e
      error "Error when handling message data: #{e.message}\n#{e.backtrace.join("\n")}", inbound_message: message
    end

    # This method returns true if the result should be stored as a new message.
    # If mode is set to 'on_change', this method may return false and update an existing
    # message to expire further in the future.
    def store_payload!(old_messages, result)
      case interpolated['mode'].presence
      when 'on_change'
        result_json = result.to_json
        if found = old_messages.find { |message| message.payload.to_json == result_json }
          found.update!(expires_at: new_message_expiration_date)
          false
        else
          true
        end
    ...
    def previous_payloads(num_messages)
      if interpolated['uniqueness_look_back'].present?
        look_back = interpolated['uniqueness_look_back'].to_i
      else
        # Larger of UNIQUENESS_FACTOR * num_messages and UNIQUENESS_LOOK_BACK
        look_back = UNIQUENESS_FACTOR * num_messages
        if look_back < UNIQUENESS_LOOK_BACK
          look_back = UNIQUENESS_LOOK_BACK
        end
      end
      messages.order('id desc').limit(look_back) if interpolated['mode'] == 'on_change'
    end
    ...
    class ResponseFromMessageDrop < LiquidDroppable::Drop
      def headers
