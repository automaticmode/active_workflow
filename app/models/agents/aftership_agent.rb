...
module Agents
  class AftershipAgent < Agent
    ...
    description <<-MD
      ...
      and replace `SLUG` with a courier code and `TRACKING_NUMBER` with the tracking number. You can request
      last checkpoint of a package by providing `last_checkpoint/SLUG/TRACKING_NUMBER` instead.
    ...
    message_description <<-MD
      A typical tracking message have 2 important objects (tracking, and checkpoint) and the tracking/checkpoint looks like this.
    ...
    def check
      response = HTTParty.get(message_url, request_options)
      messages = JSON.parse response.body
      create_message payload: messages
    end
