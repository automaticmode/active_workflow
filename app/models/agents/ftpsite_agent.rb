  ...
  class FtpsiteAgent < Agent
    ...
    description do
      <<-MD
        ...
        `mode` must be present and either `read` or `write`, in `read` mode the agent checks the FTP site for changed files, with `write` it writes received messages to a file on the server.
        ...
        Only files with a last modification time later than the `after` value, if specifed, are emitted as message.
    ...
    def saving_entries
      ...
      new_files.sort_by { |filename|
        ...
        create_message payload: get_file_pointer(filename).merge({
