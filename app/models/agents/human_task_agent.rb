module Agents
  class HumanTaskAgent < Agent
    ...
    description <<-MD
      ...
      HITs can be created in response to messages, or on a schedule.  Set `trigger_on` to either `schedule` or `message`.
      ...
      By default, all answers are emitted in a single message.  If you'd like separate messages for each answer, set `separate_answers` to `true`.
      ...
      Resulting messages will have the original `answers`, as well as the `poll` results, and a field called `best_answer` that contains the best answer as determined by the poll.  (Note that `separate_answers` won't work when doing a poll.)
    ...
    def validate_options
      ...
      errors.add(:base, "'trigger_on' must be one of 'schedule' or 'message'") unless %w[schedule message].include?(options['trigger_on'])
      ...
      if options['trigger_on'] == 'message'
        errors.add(:base, "'expected_receive_period_in_days' is required when 'trigger_on' is set to 'message'") unless options['expected_receive_period_in_days'].present?
    ...
    if defined?(RTurk)
      ...
      def message_for_hit(hit_id)
        if memory['hits'][hit_id].is_a?(Hash)
          Message.find_by_id(memory['hits'][hit_id]['message_id'])
        else
          nil
        end
      end
      ...
      def review_hits
        ...
        my_reviewed_hit_ids.each do |hit_id|
          ...
          if assignments.length == hit.max_assignments && assignments.all? { |assignment| assignment.status == 'Submitted' }
            inbound_message = message_for_hit(hit_id)
            ...
            if hit_type(hit_id) == 'poll'
              ...
              message = create_message payload: payload
              log 'Message emitted with answer(s) for poll', outbound_message: message, inbound_message: inbound_message
            else
              ...
              if create_poll?
                ...
                poll_hit = create_hit 'title' => options['poll_options']['title'],
                                      ...
                                      'payload' => inbound_message && inbound_message.payload,
                                      'metadata' => { 'type' => 'poll',
                                                      'original_hit' => hit_id,
                                                      'answers' => assignments.map(&:answers),
                                                      'message_id' => inbound_message && inbound_message.id }

                log "Poll HIT created with ID #{poll_hit.id} and URL #{poll_hit.url}.  Original HIT: #{hit_id}", inbound_message: inbound_message
              else
                if options[:separate_answers]
                  payload['answers'].each.with_index do |answer, index|
                    ...
                    message = create_message payload: sub_payload
                    log "Message emitted with answer ##{index}", outbound_message: message, inbound_message: inbound_message
                  end
                else
                  message = create_message payload: payload
                  log 'Message emitted with answer(s)', outbound_message: message, inbound_message: inbound_message
                end
              end
            end
      ...
      def create_basic_hit(message = nil)
        ...
        log "HIT created with ID #{hit.id} and URL #{hit.url}", inbound_message: message
      end
