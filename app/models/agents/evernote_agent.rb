module Agents
  class EvernoteAgent < Agent
    ...
    description <<-MD
      ...
      Options:
        ...
        * `mode` - Two possible values:
            ...
            - `update` Based on messages it receives, the agent will create notes
                       or update notes with the same `title` and `notebook`

            - `read`   On a schedule, it will generate messages containing data for newly
    ...
    message_description <<-MD
      When `mode` is `update`, messages look like:
      ...
      When `mode` is `read`, messages look like:
    ...
    def check
      if options[:mode] == 'read'
        ...
        notes.each do |note|
          ...
          create_message payload: note.attr(include_resources: true, include_content: include_xhtml_content?)
        end
