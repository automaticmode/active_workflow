module Agents
  class RssAgent < Agent
    ...
    DEFAULT_MESSAGES_ORDER = [['{{date_published}}', 'time'], ['{{last_updated}}', 'time']]

    display_name 'RSS Agent'
    ...
    description do
      <<-MD
        ...
        Options:
          ...
          * `include_feed_info` - Set to `true` to include feed information in each message.
          ...
          * `max_messages_per_run` - Limit number of messages created (items parsed) per run for feed.
    ...
    def default_options
      {
        ...
        'clean' => 'false',
        'url' => 'https://github.com/automaticmode/active_workflow/commits/master.atom'
      }
    end
    ...
    def validate_options
      ...
      validate_messages_order
    end

    def messages_order(key = SortableMessages::MESSAGES_ORDER_KEY)
      if key == SortableMessages::MESSAGES_ORDER_KEY
        super.presence || DEFAULT_MESSAGES_ORDER
      else
    ...
    def check_urls(urls)
      new_messages = []
      max_messages = (interpolated['max_messages_per_run'].presence || 0).to_i
      ...
      urls.each do |url|
        begin
          ...
          if response.success?
            ...
            new_messages.concat feed_to_messages(feed)
          else
      ...
      messages = sort_messages(new_messages).select.with_index { |message, index|
        check_and_track(message.payload[:id]) &&
          !(max_messages && max_messages > 0 && index >= max_messages)
      }
      create_messages(messages)
      log "Fetched #{urls.to_sentence} and created #{messages.size} message(s)."
    end
    ...
    def feed_to_messages(feed)
      ...
      feed.entries.map { |entry|
        Message.new(payload: payload_base.merge(entry_data(entry)))
      }
    end
