module Agents
  class S3Agent < Agent
    ...
    description do
      <<-MD
        Can watch a bucket for changes or emit a message for every file in that bucket.  When receiving messages, it writes the data into a file on S3.
        ...
        Specify the filename to use in `filename`, Liquid interpolation is possible to change the name per message.

        Use Liquid templating in `data` to specify which part of the received message should be written.
    ...
    message_description do
      "Messages will looks like this:\n\n    %s" % if boolify(interpolated['watch'])
                                                     Utils.pretty_print({
                                                                          ...
                                                                          'message_type' => 'modified/added/removed'
                                                                        })
                                                   else
                                                     Utils.pretty_print({
                                                                          'file_pointer' => {
    ...
    def validate_options
      if options['mode'].blank? || !%w[read write].include?(options['mode'])
    ...
    def check
      ...
      if boolify(interpolated['watch'])
        ...
        contents.each do |key, _|
          create_message payload: get_file_pointer(key)
        end
      end
    end
    ...
    def watch(contents)
      ...
      memory['seen_contents'].each do |key, etag|
        if contents[key].blank?
          create_message payload: get_file_pointer(key).merge(message_type: :removed)
        elsif contents[key] != etag
          create_message payload: get_file_pointer(key).merge(message_type: :modified)
        end
        contents.delete(key)
      end
      contents.each do |key, etag|
        create_message payload: get_file_pointer(key).merge(message_type: :added)
      end
