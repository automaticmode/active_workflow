...
describe AgentRunner do
  context 'without traps' do
    ...
    context '#load_workers' do
      before do
        AgentRunner.class_variable_set(:@@agents, [ActiveWorkflowScheduler, DelayedJobWorker])
      end

      it 'loads all workers' do
        workers = @agent_runner.send(:load_workers)
        expect(workers).to be_a(Hash)
        expect(workers.keys).to eq(%w[ActiveWorkflowScheduler DelayedJobWorker])
      end

      it 'loads only the workers specified in the :only option' do
        agent_runner = AgentRunner.new(only: ActiveWorkflowScheduler)
        workers = agent_runner.send(:load_workers)
        expect(workers.keys).to eq(['ActiveWorkflowScheduler'])
        agent_runner.stop
      end

      it 'does not load workers specified in the :except option' do
        agent_runner = AgentRunner.new(except: ActiveWorkflowScheduler)
        workers = agent_runner.send(:load_workers)
    ...
    context 'running workers' do
      before do
        AgentRunner.class_variable_set(:@@agents, [ActiveWorkflowScheduler, DelayedJobWorker])
        stub.instance_of(ActiveWorkflowScheduler).setup
        stub.instance_of(DelayedJobWorker).setup
      end

      context '#run_workers' do
        it 'runs all the workers' do
          mock.instance_of(ActiveWorkflowScheduler).run!
        ...
        it 'kills no long active workers' do
          ...
          mock.instance_of(ActiveWorkflowScheduler).stop!
          @agent_runner.send(:run_workers)
        end
      end
      ...
      context '#restart_dead_workers' do
        ...
        it 'restarts dead workers' do
          stub.instance_of(ActiveWorkflowScheduler).thread { OpenStruct.new(alive?: false) }
          mock.instance_of(ActiveWorkflowScheduler).run!
