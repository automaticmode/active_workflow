...
require 'active_workflow_scheduler'

describe ActiveWorkflowScheduler do
  before(:each) do
    @rufus_scheduler = Rufus::Scheduler.new
    @scheduler = ActiveWorkflowScheduler.new
  ...
  it 'should propagate messages' do
    mock(Agent).receive!
    stub.instance_of(IO).puts
    @scheduler.send(:propagate!)
  end

  it 'schould clean up expired messages' do
    mock(Message).cleanup_expired!
    stub.instance_of(IO).puts
    @scheduler.send(:cleanup_expired_messages!)
  end
  ...
  describe '#hour_to_schedule_name' do
    ...
    it 'for the forenoon' do
      expect(@scheduler.send(:hour_to_schedule_name, 6)).to eq('6h')
    end

    it 'for 12h' do
      expect(@scheduler.send(:hour_to_schedule_name, 12)).to eq('12h')
    end

    it 'for the afternoon' do
      expect(@scheduler.send(:hour_to_schedule_name, 17)).to eq('17h')
    end
  end
  ...
  context '#setup_worker' do
    it 'should return an array with an instance of itself' do
      workers = ActiveWorkflowScheduler.setup_worker
      expect(workers).to be_a(Array)
      expect(workers.first).to be_a(ActiveWorkflowScheduler)
      expect(workers.first.id).to eq('ActiveWorkflowScheduler')
    end
  end
end
