...
describe LiquidInterpolatable::Filters do
  ...
  context 'as_object' do
    ...
    it 'returns an array that was splitted in liquid tags' do
      ...
      expect(agent.interpolated['array']).to eq(%w[test string abc])
    end
    ...
    context 'as_json' do
      ...
      it 'works with MessageDrops' do
        message = Message.new(payload: { some: 'payload' }, agent: agent, created_at: Time.now)
        agent.interpolation_context['something'] = message
        agent.options['object'] = '{{something | as_object}}'
        expect(agent.interpolated['object']).to eq(ensure_safety(message.to_liquid.as_json.stringify_keys))
      end
