...
describe Agents::EvernoteAgent do
  ...
  describe '#receive' do
    context "when mode is set to 'update'" do
      before do
        ...
        @message.payload = { title: 'xkcd Survey',
      ...
      it 'adds a note for any payload it receives' do
        ...
        expect(@agent.messages.first.payload).to eq({
                                                      'title' => 'xkcd Survey',
                                                      'notebook' => 'xkcd',
                                                      'tags' => %w[funny data],
                                                      'source' => nil,
    ...
    context "when mode is set to 'read'" do
      ...
      context 'the first time it checks' do
        it 'returns only notes created/updated since it was created' do
          expect { @checker.check }.to change { Message.count }.by(0)
        end
      end
      ...
      context 'on subsequent checks' do
        ...
        it 'returns notes tagged since the last time it checked' do
          ...
          en_note_store.getNote(2).tagNames = %w[funny comic]
  ...
  describe Agents::EvernoteAgent::NoteStore::Search do
    ...
    let(:note1) {
      OpenStruct.new(title: 'first note', notebookGuid: 1, tagNames: %w[funny comic], updated: Time.now)
    }    
    let(:note2) {
      OpenStruct.new(title: 'second note', tagNames: %w[funny comic], updated: Time.now)
    }    
    let(:note3) {
    ...
    let(:search) do
      Agents::EvernoteAgent::NoteStore::Search.new(note_store,
                                                   { tagNames: %w[funny comic], notebook: 'xkcd' })
    end
    ...
    let(:search_with_time_and_tags) do
      Agents::EvernoteAgent::NoteStore::Search.new(note_store,
                                                   { notebook: 'xkcd', tagNames: %w[funny comic], notes_with_tags: [1], last_checked_at: Time.now - 1.minute })
    end
  ...
  describe Agents::EvernoteAgent::Note do
    ...
    describe '#attr' do
      let(:note) {
        Agents::EvernoteAgent::Note.new(en_note, 'xkcd', %w[funny data])
      }
