...
describe Agents::BasecampAgent do
  ...
  before(:each) do
    stub_request(:get, /messages.json$/).to_return(
  ...
  describe 'helpers' do
    it 'should generate a correct request options hash' do
      expect(@checker.send(:request_options)).to eq({ headers: { 'User-Agent' => 'ActiveWorkflow - https://github.com/automaticmode/active_workflow', 'Authorization' => 'Bearer "1234token"' } })
    end

    it 'should generate the correct messages url' do
      expect(@checker.send(:messages_url)).to eq('https://basecamp.com/12345/api/v1/projects/6789/messages.json')
    end
    ...
    it 'should provide the since attribute after the first run' do
      ...
      @checker.memory[:last_message] = time
      @checker.save
  ...
  describe '#check' do
    it 'should not emit messages on its first run' do
      expect { @checker.check }.to change { Message.count }.by(0)
      expect(@checker.memory[:last_message]).to eq '2014-04-17T10:25:31.000+02:00'
    end
    it 'should check that initial run creates an message' do
