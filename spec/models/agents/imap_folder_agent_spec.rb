...
describe Agents::ImapFolderAgent do
  ...
  describe 'checking IMAP' do
    ...
    let(:mails) {
      ...
      stub(attachment).filename.returns('filename.txt')
      stub(attachment).decoded.returns('content')
      [
        ...
        Mail.read(Rails.root.join('spec/data_fixtures/imap2.eml')).tap { |mail|
          ...
          stub(mail).attachments.returns([attachment])
          stub(mail).raw_mail.returns(mail.encoded)
        }
      ]
    }
    ...
    before do
      @checker = Agents::ImapFolderAgent.new(name: 'Example', options: valid_options, keep_messages_for: 2.days)
    ...
    describe 'validations' do
      ...
      it 'should validate the boolean fields' do
        %w[ssl mark_as_read include_raw_mail include_attachments].each do |key|
    ...
    describe '#check' do
      it 'should check for mails and save memory' do
        expect { @checker.check }.to change { Message.count }.by(2)
        ...
        expect(Message.last(2).map(&:payload)).to eq expected_payloads
      ...
      it 'should narrow mails by To' do
        ...
        expect(Message.last.payload).to eq(expected_payloads.first)
      ...
      it 'should perform regexp matching and save named captures' do
        ...
        expect(Message.last.payload).to eq(expected_payloads.last.update(
      ...
      it 'should narrow mails by has_attachment (true)' do
        ...
        expect(Message.last.payload['subject']).to eq('Re: some subject')
      end
      ...
      it 'should narrow mails by has_attachment (false)' do
        ...
        expect(Message.last.payload['subject']).to eq('some subject')
      end
      ...
      it 'should create just one message for multiple mails with the same Message-Id' do
      ...
      describe 'with include_attachments' do
        it 'should return attachment as a file pointer' do
          @checker.options['include_attachments'] = true
          @checker.save!
          @checker.check
          message = Message.last
          expect(@checker.get_io(message).read).to eq 'content'
        end
      end
      ...
      describe 'with include_raw_mail' do
        ...
        it 'should check for mails and emit messages with raw_mail' do
          expect { @checker.check }.to change { Message.count }.by(2)
          expect(@checker.notified.sort).to eq(mails.map(&:message_id).sort)
          expect(@checker.lastseen).to eq(mails.each_with_object(@checker.make_seen) { |mail, seen|
                                            seen[mail.uidvalidity] = mail.uid
          ...
          expect(Message.last(2).map(&:payload)).to match expected_payloads.map.with_index { |payload, i|
            payload.merge(
