...
describe Agents::S3Agent do
  ...
  context '#check' do
    context 'not watching' do
      it 'emits an message for every file' do
        ...
        expect(Message.last.payload).to eq({ 'file_pointer' => { 'file' => 'test2', 'agent_id' => @checker.id } })
      end
    end
    ...
    context 'watching' do
      ...
      it 'does not emit any messages on the first run' do
      ...
      context 'detecting changes' do
        ...
        it 'emits messages for removed files' do
          contents = { 'test' => '231232' }
          mock(@checker).get_bucket_contents { contents }
          expect { @checker.check }.to change(Message, :count).by(1)
          expect(Message.last.payload).to eq({ 'file_pointer' => { 'file' => 'test2', 'agent_id' => @checker.id }, 'message_type' => 'removed' })
        end

        it 'emits messages for modified files' do
          contents = { 'test' => '231232', 'test2' => 'changed' }
          mock(@checker).get_bucket_contents { contents }
          expect { @checker.check }.to change(Message, :count).by(1)
          expect(Message.last.payload).to eq({ 'file_pointer' => { 'file' => 'test2', 'agent_id' => @checker.id }, 'message_type' => 'modified' })
        end
        it 'emits messages for added files' do
          contents = { 'test' => '231232', 'test2' => '4564545', 'test3' => '31231231' }
          mock(@checker).get_bucket_contents { contents }
          expect { @checker.check }.to change(Message, :count).by(1)
          expect(Message.last.payload).to eq({ 'file_pointer' => { 'file' => 'test3', 'agent_id' => @checker.id }, 'message_type' => 'added' })
        end
      end
  ...
  context '#message_description' do
    it 'should include message_type when watch is set to true' do
      @checker.options['watch'] = 'true'
      expect(@checker.message_description).to include('message_type')
    end

    it 'should not include message_type when watch is set to false' do
      @checker.options['watch'] = 'false'
      expect(@checker.message_description).not_to include('message_type')
    end
  end
