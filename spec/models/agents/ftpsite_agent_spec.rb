...
describe Agents::FtpsiteAgent do
  describe 'checking anonymous FTP' do
    before do
      ...
      @checker = Agents::FtpsiteAgent.new(name: 'Example', options: @site, keep_messages_for: 2.days)
    ...
    describe '#check' do
      ...
      it 'should check for changes and save known entries in memory' do
        expect { @checker.check }.to change { Message.count }.by(3)
        ...
        expect(Message.last(2).first.payload).to eq({
