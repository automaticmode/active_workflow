...
describe Agents::HumanTaskAgent do
  ...
  describe 'validations' do
    it "validates that trigger_on is 'schedule' or 'message'" do
      @checker.options['trigger_on'] = 'foo'
      expect(@checker).not_to be_valid
    end

    it "requires expected_receive_period_in_days when trigger_on is set to 'message'" do
      @checker.options['trigger_on'] = 'message'
  ...
  describe 'creating hits' do
    it 'can create HITs based on messages, interpolating their values' do
      ...
      @checker.send :create_basic_hit, @message
      ...
      expect(@checker.memory['hits'][123]['message_id']).to eq(@message.id)
    end

    it 'works without an message too' do
      @checker.options['hit']['title'] = 'Hi {{name}}'
      hitInterface = OpenStruct.new
      hitInterface.id = 123
  ...
  describe 'reviewing HITs' do
    ...
    it 'should work on multiple HITs' do
      ...
      message2.payload = { 'foo2' => { 'bar2' => { 'baz2' => 'a2b2' } },
                           'name2' => 'Joe2' }
      message2.id = 3452

      # It knows about two HITs from two different messages.
      @checker.memory['hits'] = {}
      @checker.memory['hits']['JH3132836336DHG'] = { 'message_id' => @message.id }
      @checker.memory['hits']['JH39AA63836DHG'] = { 'message_id' => message2.id }
    ...
    it "shouldn't do anything if an assignment isn't ready" do
      ...
      expect(@checker.memory['hits']).to eq({ 'JH3132836336DHG' => { 'message_id' => @message.id } })
    end
    ...
    context 'emitting messages' do
      ...
      it 'should create messages when all assignments are ready' do
        expect {
          @checker.send :review_hits
        ...
        expect(@checker.messages.last.payload['answers']).to eq([
      ...
      it 'should emit separate answers when options[:separate_answers] is true' do
        ...
        message1, message2 = @checker.messages.last(2)
        expect(message1.payload).not_to have_key('answers')
        expect(message2.payload).not_to have_key('answers')
        expect(message1.payload['answer']).to eq({ 'sentiment' => 'happy', 'feedback' => 'Take 2' })
        expect(message2.payload['answer']).to eq({ 'sentiment' => 'neutral', 'feedback' => '' })
    ...
    describe 'taking majority votes' do
      ...
      it 'should take the majority votes of all questions' do
        ...
        expect(@checker.messages.last.payload['counts']).to eq({ 'sentiment' => { 'happy' => 2, 'sad' => 1, 'neutral' => 1 }, 'age_range' => { '>50' => 3, '<50' => 1 } })
        expect(@checker.messages.last.payload['majority_answer']).to eq({ 'sentiment' => 'happy', 'age_range' => '>50' })
        expect(@checker.messages.last.payload).not_to have_key('average_answer')
      ...
      it 'should also provide an average answer when all questions are numeric' do
        ...
        expect(@checker.messages.last.payload['counts']).to eq({ 'rating' => { '1' => 1, '2' => 2, '3' => 1, '4' => 0, '5.1' => 1 } })
        expect(@checker.messages.last.payload['majority_answer']).to eq({ 'rating' => '2' })
        expect(@checker.messages.last.payload['average_answer']).to eq({ 'rating' => (1 + 2 + 2 + 3 + 5.1) / 5.0 })
    ...
    describe 'creating and reviewing polls' do
      ...
      it 'creates a poll using the row_template, message, and correct number of assignments' do
        ...
        expect {
          @checker.send :review_hits
        }.to change { Message.count }.by(0) # it does not emit an message until all poll results are in
        ...
        expect(@checker.memory['hits']['JH39AA63836DH12345']['message_id']).to eq(@message.id)
      ...
      it 'emits an message when all poll results are in, containing the data from the best answer, plus all others' do
        original_answers = [
        ...
        expect(@checker.messages.last.payload['answers']).to eq(original_answers)
        expect(@checker.messages.last.payload['poll']).to eq([{ '1' => '2', '2' => '5', '3' => '3', '4' => '2' }, { '1' => '3', '2' => '4', '3' => '1', '4' => '4' }])
        expect(@checker.messages.last.payload['best_answer']).to eq({ 'sentiment' => 'neutral', 'feedback' => 'This is my feedback 2' })
