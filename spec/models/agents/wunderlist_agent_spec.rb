...
describe Agents::WunderlistAgent do
  ...
  before(:each) do
    ...
    @message.payload = { title: 'hello', url: 'www.example.com' }
    @message.save!
  end
  ...
  it 'should generate the request_options' do
    expect(@checker.send(:request_options)).to eq({ headers: { 'Content-Type' => 'application/json', 'User-Agent' => 'ActiveWorkflow - https://github.com/automaticmode/active_workflow', 'X-Access-Token' => '1234token', 'X-Client-ID' => 'wunderoauthkey' } })
  end
  ...
  describe '#complete_list_id' do
    it 'should return a array of hashes' do
      stub_request(:get, 'https://a.wunderlist.com/api/v1/lists').to_return(
        body: JSON.dump([{ title: 'test', id: 12_345 }]),
        headers: { 'Content-Type' => 'text/json' }
      )
      expect(@checker.complete_list_id).to eq([{ text: 'test (12345)', id: 12_345 }])
    end
  end
