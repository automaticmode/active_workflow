...
describe Agents::PostAgent do
  ...
  describe '#receive' do
    it "can handle multiple messages and merge the payloads with options['payload']" do
      ...
      expect(@sent_requests[:post][0].data).to eq(@message.payload.merge('default' => 'value').to_query)
      expect(@sent_requests[:post][1].data).to eq(message1.payload.to_query)
    end
    ...
    it 'can make GET requests' do
      ...
      expect(@sent_requests[:get][0].data).to eq(@message.payload.merge('default' => 'value').to_query)
    end

    it 'can make a GET request merging params in post_url, payload and message' do
      @checker.options['method'] = 'get'
    ...
    it 'can skip merging the incoming message when no_merge is set, but it still interpolates' do
    ...
    it 'interpolates outgoing headers with the message payload' do
    ...
    it 'makes a multipart request when receiving a file_pointer' do
      ...
      stub_request(:post, 'http://www.example.com/')
        .with(headers: {
                ...
                'User-Agent' => 'ActiveWorkflow - https://github.com/automaticmode/active_workflow'
              }) { |request|
  ...
  describe '#check' do
    ...
    describe 'emitting messages' do
      context 'when emit_messages is not set to true' do
        it 'does not emit messages' do
          expect {
            @checker.check
          }.not_to change { @checker.messages.count }
        end
      end

      context 'when emit_messages is set to true' do
        before do
        ...
        it 'emits the response status' do
          expect {
            @checker.check
          }.to change { @checker.messages.count }.by(1)
          expect(@checker.messages.last.payload['status']).to eq 200
        end
        ...
        it 'emits the response headers capitalized by default' do
          @checker.check
          expect(@checker.messages.last.payload['headers']).to eq({ 'Content-Type' => 'text/html' })
        end

        it 'emits the response headers capitalized' do
          @checker.options['message_headers_style'] = 'capitalized'
          @checker.check
          expect(@checker.messages.last.payload['headers']).to eq({ 'Content-Type' => 'text/html' })
        end

        it 'emits the response headers downcased' do
          @checker.options['message_headers_style'] = 'downcased'
          @checker.check
          expect(@checker.messages.last.payload['headers']).to eq({ 'content-type' => 'text/html' })
        end

        it 'emits the response headers snakecased' do
          @checker.options['message_headers_style'] = 'snakecased'
          @checker.check
          expect(@checker.messages.last.payload['headers']).to eq({ 'content_type' => 'text/html' })
        end
  ...
  describe 'validation' do
    ...
    it 'requires emit_messages to be true or false' do
      @checker.options['emit_messages'] = 'what?'
      expect(@checker).not_to be_valid

      @checker.options.delete('emit_messages')
      expect(@checker).to be_valid

      @checker.options['emit_messages'] = 'true'
      expect(@checker).to be_valid

      @checker.options['emit_messages'] = 'false'
      expect(@checker).to be_valid
