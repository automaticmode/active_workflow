...
describe Agents::JiraAgent do
  ...
  describe 'helpers' do
    it 'should generate a correct request options hash' do
      expect(@checker.send(:request_options)).to eq({ basic_auth: { username: 'user', password: 'pass' }, headers: { 'User-Agent' => 'ActiveWorkflow - https://github.com/automaticmode/active_workflow' } })
    end
    ...
    it "should not set the 'since' time on the first run" do
      ...
      expected_headers = { headers: { 'User-Agent' => 'ActiveWorkflow - https://github.com/automaticmode/active_workflow' }, basic_auth: { username: 'user', password: 'pass' } }
  ...
  describe '#check' do
    it 'should be able to retrieve issues' do
      ...
      expect { @checker.check }.to change { Message.count }.by(50)
    end
  end

  describe '#working?' do
    it 'it is working when at least one message was emited' do
