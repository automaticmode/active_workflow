...
describe Agents::AftershipAgent do
  ...
  describe '#helpers' do
    ...
    it 'should generate the correct messages url' do
      expect(@checker.send(:message_url)).to eq('https://api.aftership.com/v4/trackings')
    end

    it 'should generate the correct specific tracking url' do
      @checker.options['path'] = 'trackings/usps/9361289878905919630610'
      expect(@checker.send(:message_url)).to eq('https://api.aftership.com/v4/trackings/usps/9361289878905919630610')
    end

    it 'should generate the correct last checkpoint url' do
      @checker.options['path'] = 'last_checkpoint/usps/9361289878905919630610'
      expect(@checker.send(:message_url)).to eq('https://api.aftership.com/v4/last_checkpoint/usps/9361289878905919630610')
    end
  end
