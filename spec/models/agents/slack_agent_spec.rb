...
describe Agents::SlackAgent do
  before(:each) do
    ...
    @message.payload = { channel: '#random', message: 'Looks like its going to rain', username: 'Huggin user', fallback: @fallback }
    @message.save!
  end
  ...
  describe '#receive' do
    it 'receive an message without errors' do
      any_instance_of(Slack::Notifier) do |obj|
        mock(obj).ping(@message.payload[:message],
                       attachments: [{ 'fallback' => @fallback }],
