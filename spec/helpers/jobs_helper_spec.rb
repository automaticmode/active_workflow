...
describe JobsHelper do
  ...
  describe '#status' do
    it 'works for failed jobs' do
      ...
      expect(status(job)).to eq('<span class="badge badge-danger">failed</span>')
    end

    it 'works for running jobs' do
      job.locked_at = Time.now
      job.locked_by = 'test'
      expect(status(job)).to eq('<span class="badge badge-info">running</span>')
    end

    it 'works for queued jobs' do
      expect(status(job)).to eq('<span class="badge badge-warning">queued</span>')
    end
  end
